...
export default function SpecialtyCard({ specialty, mobile = false }) {
  ...
  return (
    ...
    >
      {customImage ? (
        <div className="w-16 h-16 sm:w-[72px] sm:h-[72px] aspect-square rounded-2xl overflow-hidden transition-transform duration-300 group-hover:-translate-y-1 group-hover:shadow-lg">
          <img src={customImage} alt="" className="w-full h-full object-cover" />
        </div>
      ) : (
        <div
