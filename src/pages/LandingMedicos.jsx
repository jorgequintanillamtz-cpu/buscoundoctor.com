    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <section aria-labelledby="cta-final-heading" className="max-w-4xl mx-auto px-4 sm:px-6 pb-16 sm:pb-20">
          <div className="relative bg-card border border-border/50 rounded-3xl overflow-hidden p-6 sm:p-10 text-center shadow-sm">
            <div className="absolute -top-14 -right-14 w-56 h-56 bg-brand-blue/10 rounded-full pointer-events-none" />
            <div className="absolute -bottom-10 -left-10 w-48 h-48 bg-brand-bluePale/50 rounded-full pointer-events-none" />
            <div className="relative">
              <h2 id="cta-final-heading" className="font-heading font-extrabold text-2xl sm:text-3xl text-foreground tracking-tight">
                Todo lo que obtienes al registrarte hoy
              </h2>

              <div className="grid sm:grid-cols-2 gap-3 max-w-xl mx-auto text-left mt-6">
                {[
                  "Perfil verificado en el directorio, gratis",
                  "1 año gratis de Premium por ser Miembro Fundador",
                  "Cédula profesional verificada",
                  "Contacto directo por WhatsApp, sin intermediarios",
                ].map((item) => (
                  <div key={item} className="flex items-start gap-2.5 bg-brand-bluePale/40 border border-brand-blue/10 rounded-2xl px-4 py-3">
                    <CheckCircle2 className="w-4 h-4 text-brand-blue flex-shrink-0 mt-0.5" />
                    <p className="text-sm text-brand-navy">{item}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-border/50">
                <p className="text-sm text-muted-foreground mb-3">¿Sigues teniendo dudas?</p>
                <a
                  href={WHATSAPP_HREF}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#1fb855] transition-colors text-white text-sm font-semibold px-5 py-3 rounded-xl"
                >
