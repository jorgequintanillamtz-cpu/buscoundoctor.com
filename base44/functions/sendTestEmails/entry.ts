...
// Funcion temporal, solo para mandar los 7 correos de muestra (con el nuevo
// diseno de marca) al buzon del dueno y que pueda revisar como quedaron.
// No se referencia desde ninguna pantalla del admin ni del panel del
// doctor - se invoca directo por URL y se borra despues de la revision.
//
// Las funciones de Base44 no pueden importar archivos fuera de su propia
// carpeta, asi que aqui va una copia minima de los helpers de
  ...
  return [
    {
      ...
      html: renderEmail({
        ...
        bodyHtml: `<p>Hola ${esc(SAMPLE_DOC.full_name)},</p><p>Buenas noticias: revisamos tu perfil y ya esta <strong>aprobado y publicado</strong> en ${SITE_NAME}. Los pacientes ya pueden encontrarte en el directorio.</p>${detailTable([
          detailRow('Doctor', esc(SAMPLE_DOC.full_name)),
          detailRow('Especialidad', esc(SAMPLE_DOC.specialty)),
          detailRow('Perfil publico', `<a href="${SITE_URL}/especialista/${SAMPLE_DOC.slug}" style="color:#2F6FED;text-decoration:none;">Ver mi perfil &rarr;</a>`),
        ])}`,
    ...
    },
    {
      ...
      html: renderEmail({
        ...
        bodyHtml: `<p>Hola ${esc(SAMPLE_DOC.full_name)},</p><p>Tu documento fue revisado y <strong>aprobado</strong>.</p>${detailTable([detailRow('Doctor', esc(SAMPLE_DOC.full_name)), detailRow('Documento', 'Cedula profesional')])}`,
    ...
    },
    {
      ...
      html: renderEmail({
        ...
        bodyHtml: `<p>Hola ${esc(SAMPLE_DOC.full_name)},</p><p>Tu articulo fue aprobado y ya esta publicado en el blog de ${SITE_NAME}.</p>${detailTable([
          ...
          detailRow('Ver publicado', `<a href="${SITE_URL}/blog/5-senales-dermatologo" style="color:#2F6FED;text-decoration:none;">Leer articulo &rarr;</a>`),
        ])}`,
        ctaLabel: 'Ir a mi panel',
        ctaUrl: PANEL_URL,
      }),
    ...
    for (const e of emails) {
      try {
        await base44.integrations.Core.SendEmail({
          to,
          subject: `[Muestra de diseno v2] ${e.subject}`,
          body: e.html,
          from_name: SITE_NAME,
        });
