...
Deno.serve(async (req) => {
  try {
    ...
    const steps = [
      stepRow(1, ICONS.cedula, 'Verifica tu cedula profesional', 'Sube tu cedula y tu identificacion oficial.', 'Subir documentos', PANEL_URL),
      stepRow(2, ICONS.biografia, 'Completa tu biografia', 'Escribe al menos 50 palabras sobre tu experiencia.', 'Escribir biografia', PANEL_URL),
    ].join('');

    const html = renderEmail({
      preheader: 'Bienvenido a BuscoUnDoctor.',
      hero: {
        eyebrow: 'Cuenta creada',
        title: `¡Bienvenido a ${SITE_NAME}, Dr. Roberto!`,
        subtitle: 'Tu perfil ya existe, pero le faltan algunos datos para verse profesional.',
      },
      title: 'Primeros pasos para completar tu perfil',
      bodyHtml: `<div style="margin:18px 0 4px;">${steps}</div>`,
      ctaLabel: 'Completar mi perfil',
      ctaUrl: PANEL_URL,
    });

    await base44.integrations.Core.SendEmail({ to, subject: '[Diseno v6] Logo en el hero de bienvenida', body: html, from_name: SITE_NAME });
