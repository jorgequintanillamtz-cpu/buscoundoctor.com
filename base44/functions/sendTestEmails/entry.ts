...
// Funcion temporal: correo de bienvenida con iconos generados y 6 pasos.
...
Deno.serve(async (req) => {
  try {
    ...
    const steps = [
      stepRow(1, ICONS.cedula, 'Verifica tu cedula profesional', 'Sube tu cedula y tu identificacion oficial. Los perfiles verificados generan mas confianza y se destacan con un sello especial.', 'Subir documentos', PANEL_URL),
      stepRow(2, ICONS.biografia, 'Completa tu biografia', 'Escribe al menos 50 palabras sobre tu experiencia y enfoque.', 'Escribir biografia', PANEL_URL),
      stepRow(3, ICONS.zona, 'Agrega tu zona de cobertura', 'Registra tu consultorio con su zona para aparecer en busquedas locales.', 'Agregar consultorio', PANEL_URL),
      stepRow(4, ICONS.formacion, 'Suma tu formacion e idiomas', 'Agrega tus estudios, certificaciones y los idiomas que hablas.', 'Completar formacion', PANEL_URL),
      stepRow(5, ICONS.aseguradoras, 'Indica las aseguradoras que aceptas', 'Ayuda a que pacientes con esa cobertura te encuentren primero.', 'Elegir aseguradoras', PANEL_URL),
      stepRow(6, ICONS.servicios, 'Agrega tus servicios y precios', 'Le da al paciente una idea clara antes de contactarte.', 'Agregar servicios', PANEL_URL),
    ].join('');
    ...
    const html = renderEmail({
      ...
      hero: {
        ...
        title: `¡Bienvenido a ${SITE_NAME}, Dr. Roberto!`,
        subtitle: 'Tu perfil ya existe, pero le faltan algunos datos para verse profesional y aparecer bien en las busquedas.',
      },
      title: 'Primeros pasos para completar tu perfil',
      bodyHtml: `<div style="margin:18px 0 4px;">${steps}</div><p style="margin:8px 0 0;font-size:13px;color:#98A2B3;">Un admin de nuestro equipo revisara tu cedula en cuanto la subas - normalmente en menos de 24 horas.</p>`,
      ctaLabel: 'Completar mi perfil',
      ctaUrl: PANEL_URL,
    });

    await base44.integrations.Core.SendEmail({ to, subject: '[Diseno v5] Bienvenido a BuscoUnDoctor - completa tu perfil', body: html, from_name: SITE_NAME });
