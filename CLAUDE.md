...
- **`public.registration_recovery_email_html(nombre, continue_url, parrafos_antes[], parrafos_despues[], firma_html)`**: la plantilla HTML compartida de los 3 correos (mismo header navy con logo, botón navy "Continuar registro", pie gris -- mismo estilo que ya usa el correo interno de `create_doctor_profile`). Los textos son los que redactó Jorge, tal cual.
...
- Migraciones aplicadas en Supabase (no están en git): `registration_recovery_email_columns`, `registration_recovery_email_html_helper`, `send_transactional_email_add_recovery_types`, `send_registration_recovery_emails_fn`, `get_registration_draft_fn`, `schedule_registration_recovery_emails_cron`, `has_newer_or_completed_registration_attempt_fn`, `fix_has_newer_attempt_whatsapp_normalization`.
