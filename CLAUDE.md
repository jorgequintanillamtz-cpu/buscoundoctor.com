...
- **`public.registration_recovery_email_html(nombre, continue_url, frase_destacada, parrafos_antes[], beneficios[], texto_boton, parrafos_despues[], firma_html)`**: la plantilla HTML compartida de los 3 correos (header navy con logo, pie gris -- mismo estilo que ya usa el correo interno de `create_doctor_profile`). Rediseño 2026-09-23 (a petición de Jorge, "que se vea más motivante"): debajo del header va una foto en círculo (`site-assets/hero-registro-recuperacion-doctor.jpg` -- foto genérica que Jorge proporcionó él mismo, **no** es de ningún doctor registrado en la plataforma; las fotos que sí suben los doctores son para su propio perfil público, no para reusarlas en este tipo de correo sin pedirles permiso. Antes de esto se probó primero con la ilustración que ya usa `/para-medicos`, estetoscopio en forma de corazón + silueta de Monterrey, en `site-assets/hero-para-medicos.png` -- se queda sin usar pero no se borró del bucket por si se quiere retomar), luego una frase destacada en negrita justo debajo del saludo, una cajita de beneficios con checks (`brand.blueLight` de fondo), y un botón más grande con flecha. Los 3 correos ya llevan la frase destacada y la cajita de beneficios (al principio el correo 3 se dejó sin eso, "más corto y personal", pero Jorge pidió parejarlo con el 1 y el 2 -- solo la firma sigue distinta, "Jorge" en vez de "Equipo BuscoUnDoctor").
