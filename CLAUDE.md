...
- **Confianza en el hero (2026-09-23):** las "enfermedades principales" de arriba ahora enlazan a `/enfermedades/:slug/:ciudad` (antes eran etiquetas sueltas sin enlace, inconsistente con la sección "Especialidades" más abajo que sí enlaza — mismo dato, dos comportamientos). Debajo de esas etiquetas se muestran hasta 4 aseguradoras que acepta (`resolvedInsurers`, con "+N más" si tiene más) — antes solo se veían dentro de "Agendar cita" o, en móvil, en un bloque aparte más abajo; "¿acepta mi seguro?" es de lo primero que decide un paciente. Y arriba del nombre se muestra "Perfil actualizado: [fecha]" (usa `specialist.updated_date`), junto con "Última verificación" cuando aplica.
- **`specialist.updated_date` no se movía nunca tras crear el perfil** (a diferencia de casi 40 tablas del sitio, `specialist` era la única sin el trigger estándar `touch_updated_date()`) — se agregó el trigger `specialist_touch` (migración aplicada en Supabase, no está en git) para que la fecha de arriba diga la verdad. Ojo: el autoguardado del panel (`useAutoSaveSpecialist`, cada 30s, envía el formulario completo sin comparar cambios) ahora también actualiza esta fecha en cada ciclo mientras el doctor tiene el panel abierto — es correcto (si escribe algo, sí se guardó), pero significa que la fecha puede decir "hoy" muy seguido para un doctor activo.
...
- Migraciones aplicadas en Supabase (no están en git): `doctor_settings_email_prefs_and_deletion_request`, `doctor_settings_notice_email_and_vacation`, `doctor_settings_reviews_email_and_data_export`, `specialist_assistant_access`, `fix_invite_assistant_crypt_schema`, `review_owner_visibility_and_doctor_reply`, `grant_anon_execute_can_write_specialist_media`.
- **Bug real encontrado y corregido (2026-09-23): ningún doctor nuevo podía subir su foto al registrarse.** Cuando se agregó `can_write_specialist_media()` (ver arriba), la función se quedó sin permiso de ejecución para el rol `anon` (solo lo tenía `authenticated`) — un caso concreto de la "Trampa de permisos en funciones de Postgres" del §5. El paso 3 del registro ("Agrega tus fotos") sube a la carpeta `pending-registro/` **sin sesión**, así que cualquier doctor nuevo real disparaba ese error 100% de las veces (verificado reproduciéndolo como visitante anónimo: `permission denied for function can_write_specialist_media`); las subidas que sí se veían en Storage eran de sesiones autenticadas (ustedes probando la vista previa del admin), no de doctores nuevos. Arreglo de una línea: `grant execute on function public.can_write_specialist_media(uuid) to anon;` — seguro porque la función igual evalúa `false` para un visitante sin cuenta, solo que ahora sin tronar. Verificado de nuevo como anónimo después del cambio: ya sube bien.
...
- **`getDoctorStateInfo(doc)`** (`src/api/doctorReview.js`): un solo lugar para "¿qué etiqueta de estado le pongo a este doctor?" (Borrador / En revisión / Con cambios pendientes / Corrigió y espera revisión / Publicado / En pausa / De vacaciones hasta…), usado tanto en `AdminDoctorReview.jsx` como en la lista de `AdminDoctores.jsx`. Antes la lista solo mostraba "Activo/Inactivo" (el interruptor `toggleActive` de ahí, que sigue igual) — un doctor nunca revisado se veía idéntico a uno pausado a propósito. Ahora la etiqueta de estado real va como chip aparte, y el nombre del doctor en la lista enlaza a `/admin/doctores/revisar/:id` (el lápiz sigue yendo al editor completo).
...
- **Panorama para el equipo** (`/admin/referidos`, `AdminReferidos.jsx`): invitaciones totales, premios acreditados, esperando acreditar, valor acreditado; ranking de quién ha invitado más; y la lista completa de referidos con el mismo botón "Acreditar" que ya existe en la Bandeja y en la revisión individual (misma RPC, tres entradas). El admin no necesita `list_my_referrals()` aquí — `is_admin()` ya le deja ver todo con `Specialist.list()` normal.
...
## 8d. Cuenta propia del dueño y análisis del Dashboard (admin dueños)

- **"Mi cuenta"** (`/admin/cuenta`, `AdminAccountSettings.jsx`, enlace en la barra lateral arriba de "Cerrar sesión" y en el header de celular): Jorge y David ya pueden cambiar su propia contraseña y cerrar sesión en todos los dispositivos, sin salir del sitio. Antes no existía de verdad — había un archivo `MiPerfil.jsx` pensado para esto, pero la ruta real (`/admin/mi-perfil`) solo redirigía a `/panel-medico`; se borró por ser código muerto. La nueva pantalla reusa tal cual `changePassword`/`hasPasswordLogin`/`signOutEverywhere` de `src/api/doctorSettings.js` (esas funciones nunca tuvieron nada específico de doctor, operan sobre la sesión de quien esté logueado) — no incluye "Tu asistente"/"Descargar mis datos"/"Dar de baja", porque esas son cosas de un perfil de médico, no de una cuenta de dueño. Todas las rutas de `/admin/*` ya están detrás de `RequireAdmin`, así que la pantalla no repite el chequeo de rol.
- **Dashboard: sección "Crecimiento"** (entre "Resumen" y "Estadísticas" en `Dashboard.jsx`): una gráfica de registros nuevos por semana (últimas 12 semanas) y dos listas de barras — doctores por especialidad y doctores por zona. La de especialidad usa `specialist.specialty` (campo vivo). La de zona **no** usa `specialist.zone` (campo legado, ver §4) — se calcula desde `office.zone_id` contra el catálogo `zone`, un doctor con consultorios en dos zonas cuenta en ambas. Antes el Dashboard solo tenía un número suelto ("registros nuevos (30 días)"), sin tendencia ni desglose — ahora, con Referidos recién lanzado, hay dónde ver si el ritmo de registros sube.
...
- **"Embudo de registro"** (2026-09-23; se agregó al Dashboard entre "Resumen" y "Crecimiento", y ese mismo día se mudó a la pantalla "Registros" -- ver §8f -- porque encaja mejor ahí que en el resumen general del negocio): cuántos empiezan a llenar `/registro-medico` en los últimos 30 días, cuántos sí terminan (crean su cuenta), y en qué paso exacto se quedan los que no — usa `specialist.registration_step` (`datos`/`ubicacion`/`fotos`), que la RPC `save_registration_draft` escribe cada vez que alguien avanza un paso del wizard **antes de tener cuenta siquiera** (el borrador vive con `publication_status='draft'` y sin `owner_user_id` hasta que `create_doctor_profile` lo reclama). Un doctor creado por otro camino (ej. el editor del admin) nunca toca esa columna, así que sirve para aislar solo intentos de autoregistro público del resto de `specialist`. **Límite a propósito:** mide desde que alguien ya llenó nombre+WhatsApp+especialidad (lo mínimo para que se guarde el primer borrador) — no sabemos cuánta gente solo abrió la página sin llegar a eso; para verlo haría falta conectar Google Analytics/Search Console (no está conectado hoy).
...
- **`StepDatos.jsx`** tiene un campo "Correo electrónico" nuevo (obligatorio, junto a WhatsApp). Se guarda en la columna `specialist.email`, que ya existía (es el mismo "correo de contacto" editable después en `DoctorEditorPerfil.jsx` — no se creó una columna nueva). Al llegar al paso 4, el campo de correo para crear la cuenta ya viene precargado con este mismo valor (`RegistroMedico.jsx`), para no pedirlo dos veces.
- **`save_registration_draft(p_payload)`** (RPC, migración `save_registration_draft_captures_email`) ahora también guarda `email` en cada paso, igual que el resto de los campos del wizard.
- **Bug real encontrado y corregido de paso** (migración `fix_save_registration_draft_empty_address_parts`): la función componía la dirección (`specialist.address`) comparando contra `is not null`, pero el wizard manda esas llaves siempre, vacías o no (nunca las omite) — así que cualquier borrador que no había llegado al paso "Ubicación" quedaba con `address = ", Int. , Piso , Col. , CP"` en vez de vacío. Se cambió a `nullif(trim(...), '') is not null`. No afecta perfiles ya completados con dirección real (se probó que sigue componiendo bien).
- **Pantalla "Registros"** (`/admin/registros`, `AdminRegistros.jsx`, menú lateral junto a "Doctores"): todos los intentos de autoregistro público, terminados o no. Se aíslan del resto de `specialist` por `registration_step is not null` (esa columna solo la toca el wizard público, nunca el editor del admin). Arriba de todo va el **"Embudo de registro"** (ver §8d -- se movió aquí desde el Dashboard el mismo día que se agregó, mismos componentes `FunnelStatCard`/`SectionLabel`, copiados tal cual porque son de uso único). Cada fila de la tabla muestra nombre, correo, especialidad y un estado ("Completado" si tiene `owner_user_id`, o "Abandonado en 'Datos/Ubicación/Fotos'" si no); el nombre (o el ícono de ojo) abre el detalle completo de lo que alcanzó a llenar (incluye el precio de consulta, que vive aparte en `specialist_service`, no en `specialist`) **y el estado de sus 3 correos de recuperación** (§8g) — enviado y cuándo, o "no enviado todavía". Filtros: estado, último paso, rango de fecha, buscar por nombre/correo/WhatsApp/especialidad.
- **No se puede ver si el doctor abrió el correo** (solo si se mandó). Verlo necesitaría habilitar "open tracking" en la cuenta de Resend y un webhook nuevo que reciba ese aviso y lo guarde -- no está construido, es trabajo aparte si se vuelve a pedir.
- **Límite conocido:** `referral_code` (código de invitación) no se guarda en el borrador -- solo `create_doctor_profile` (el paso final) lo resuelve a `referred_by_id`. Si alguien pone un código y abandona antes de crear la cuenta, ese dato se pierde; no se muestra en el detalle de un registro abandonado.

## 8g. Serie de correos para recuperar registros abandonados (2026-09-23)

Tres correos automáticos (30 min / 24h después del 1° / 4 días después del 2°) a quien empieza el registro público y no lo termina. Todo vive en Supabase (Postgres), reusando la infraestructura que ya existía — **no se agregó ningún servicio ni dependencia nueva**.

- **`public.send_registration_recovery_emails()`**: la corre `pg_cron` cada 10 minutos (job `send-registration-recovery-emails`, mismo mecanismo que `publish-scheduled-posts` y `end-expired-vacations`). Revisa 3 colas independientes y por cada doctor que le toca, primero hace `UPDATE ... SET recovery_email_N_sent_at = now() WHERE recovery_email_N_sent_at IS NULL` -- si esa actualización no "gana" la fila (ya se mandó, o dos corridas chocaron), no manda nada. Así es idempotente sin necesidad de un checklist aparte.
  - Email 1: `registration_step is not null`, `owner_user_id is null`, `email is not null`, `created_date` hace ≥30 min, `recovery_email_1_sent_at is null`.
  - Email 2: igual pero `recovery_email_1_sent_at` hace ≥24h y `recovery_email_2_sent_at is null`.
  - Email 3: igual pero `recovery_email_2_sent_at` hace ≥4 días y `recovery_email_3_sent_at is null`.
  - **Se detiene solo:** las 3 consultas exigen `owner_user_id is null` -- en cuanto el doctor termina su registro, sale de las tres colas en la siguiente corrida. No hay que cancelar nada a mano.
- **3 columnas nuevas en `specialist`**: `recovery_email_1_sent_at` / `_2_sent_at` / `_3_sent_at` (timestamptz, nullable). No hay tabla aparte -- `created_date` ya es "cuándo empezó" y `owner_user_id` ya es "¿terminó?".
- **`send_transactional_email`** (la RPC que ya mandaba todos los demás correos del sitio) ahora también acepta los tipos `registro_recuperacion_1/2/3` en su lista blanca. Como esa función ya resolvía el destinatario con `coalesce(u.email, s.email)`, **ya sabía mandarle correo a un borrador sin cuenta todavía** -- no hubo que tocar su lógica de envío. Cada intento queda en `email_log` igual que cualquier otro correo del sitio (para qué, a quién, si Resend respondió bien).
- **`public.registration_recovery_email_html(nombre, continue_url, foto_url, frase_destacada, parrafos_antes[], beneficios[], texto_boton, parrafos_despues[], firma_html)`**: la plantilla HTML compartida de los 3 correos (header navy con logo, pie gris -- mismo estilo que ya usa el correo interno de `create_doctor_profile`). Rediseño 2026-09-23 (a petición de Jorge, "que se vea más motivante"): debajo del header va una foto en círculo, una frase destacada en negrita justo debajo del saludo, una cajita de beneficios con checks (`brand.blueLight` de fondo), y un botón más grande con flecha. Los 3 correos ya llevan la frase destacada y la cajita de beneficios (al principio el correo 3 se dejó sin eso, "más corto y personal", pero Jorge pidió parejarlo con el 1 y el 2 -- solo la firma sigue distinta, "Jorge" en vez de "Equipo BuscoUnDoctor").
  - **Cada correo trae su propia foto** (`p_photo_url`, antes era una sola compartida) -- fotos genéricas que Jorge proporcionó él mismo, **no** son de ningún doctor registrado en la plataforma (las fotos que sí suben los doctores son para su propio perfil público, no para reusarlas aquí sin pedirles permiso). Correo 1: `site-assets/hero-registro-recuperacion-doctor-1.jpg`. Correo 2: `site-assets/hero-registro-recuperacion-doctor-2.jpg` (se reemplazó una vez, de fondo transparente a fondo blanco -- el archivo `-2.webp` quedó huérfano en el bucket, sin borrar). Correo 3: `site-assets/hero-registro-recuperacion-doctor-3.jpg` (la primera foto que Jorge había mandado, al principio). La ilustración que se usó antes de tener fotos reales (estetoscopio en forma de corazón + silueta de Monterrey, la misma de `/para-medicos`) se queda sin usar en `site-assets/hero-para-medicos.png`, sin borrar por si se retoma.
  - Al agregarle un parámetro nuevo a esta función (`p_photo_url`) quedaron dos firmas viejas huérfanas (Postgres trata un parámetro de más como una función distinta, no como un reemplazo) -- se borraron a mano con `DROP FUNCTION`. **Si vuelves a cambiarle los parámetros a esta función, no olvides borrar la firma vieja** o se quedan las dos vivas.
- **El link "Continuar registro"** (`https://buscoundoctor.com/registro-medico?draft=ID`) **resuelve algo que antes no existía**: hasta ahora, ni recargando la página en el mismo celular el formulario se rellenaba solo (solo se recordaba el id del borrador para no duplicarlo, el formulario visible arrancaba en blanco). Ahora:
  - **`public.get_registration_draft(uuid)`** (RPC, `SECURITY DEFINER`, `EXECUTE` otorgado a `anon` y `authenticated` a propósito -- ver la "Trampa de permisos" del §5, aquí es al revés: sin el grant, un visitante sin sesión no puede llamarla) regresa los campos del borrador dado su id. El id es la única llave (imposible de adivinar, mismo principio que un link de "recuperar contraseña") -- **si el borrador ya tiene `owner_user_id` (ya creó su cuenta), regresa vacío**, para que el link de un registro ya terminado no exponga nada.
  - `RegistroMedico.jsx` detecta `?draft=` en la URL cuando **no** hay sesión, llama esa RPC, rellena `data` con lo que ya había escrito, y salta directo al paso donde se quedó (según `registration_step`: "datos" → abre en "Ubicación"; "ubicacion" → abre en "Fotos"; "fotos" → abre en "Cuenta"). Si el id no existe o ya está reclamado, sigue igual que siempre (formulario vacío), nunca bloquea el registro.
- **Cómo probar sin esperar 30 min/24h/4 días:** crear una fila `specialist` de prueba con `created_date`/`recovery_email_N_sent_at` ya "atrasados" (`now() - interval '40 minutes'`, etc.) y correr `select public.send_registration_recovery_emails();` a mano -- revisa `email_log` para confirmar el envío. Para simular que un doctor de prueba "ya terminó" (probar que deja de recibir correos), hay que poner `owner_user_id` con `select set_config('app.bypass_score_protection', 'true', true);` antes del `UPDATE` (el trigger `protect_specialist_admin_columns` revierte ese campo en cualquier `UPDATE` normal fuera de una RPC autorizada -- ver §8a).
- **Nada nuevo que configurar:** el remitente (`notificaciones@mail.buscoundoctor.com`, dominio ya verificado en Resend) y la llave (`vault.decrypted_secrets` → `resend_api_key`) ya existían y se reusan tal cual. Ninguna variable de entorno nueva.
- **Un doctor que intenta el registro varias veces (otro dispositivo, borró su navegador, etc.) no recibe correos de más ni de menos.** `public.has_newer_or_completed_registration_attempt(specialist_id)` conecta intentos por **correo o WhatsApp** (nunca por nombre -- dos doctores distintos pueden llamarse parecido, y mezclarlos sería un error real; correo/WhatsApp ya son obligatorios desde el paso 1). Las 3 colas de `send_registration_recovery_emails()` excluyen a un borrador si, para ese mismo correo o WhatsApp, existe **otro** registro que: (a) ya se completó (el borrador viejo queda obsoleto aunque nadie lo haya tocado), o (b) es más nuevo que él (solo le sigue la serie de correos al intento más reciente). El WhatsApp se compara por sus últimos 10 dígitos (mismo criterio que el bug del "52 duplicado" ya conocido en el proyecto, para que "8112345678" y "528112345678" cuenten como el mismo número).
- Migraciones aplicadas en Supabase (no están en git): `registration_recovery_email_columns`, `registration_recovery_email_html_helper`, `send_transactional_email_add_recovery_types`, `send_registration_recovery_emails_fn`, `get_registration_draft_fn`, `schedule_registration_recovery_emails_cron`, `has_newer_or_completed_registration_attempt_fn`, `fix_has_newer_attempt_whatsapp_normalization`, `registration_recovery_email_html_v2_more_motivating`, `send_registration_recovery_emails_v2_more_motivating`, `send_registration_recovery_emails_v3_email3_same_style`, `registration_recovery_email_html_v4_add_hero_image`, `registration_recovery_email_html_v5_jorge_photo`, `registration_recovery_email_html_v6_per_email_photo`, `drop_old_registration_recovery_email_html_signature`, `drop_oldest_registration_recovery_email_html_signature`, `send_registration_recovery_emails_v4_per_email_photo`, `send_registration_recovery_emails_v5_final_photos`.

## 8h. Correo de seguimiento "completa tu perfil" (2026-09-23)

Correo aparte de la serie de recuperación del §8g: **no** es para quien abandonó el registro, es para el doctor que **ya terminó** (ya tiene cuenta y perfil creado) y le recuerda que puede seguir llenando su perfil (formación, seguros, idiomas, fotos del consultorio, casos de éxito -- lo mismo que ya se le mostraba en la pantalla "Esto apenas empieza" justo al terminar el registro). Sale **1 día después de crear la cuenta**, una sola vez.

- **`specialist.account_created_at`**: columna nueva, la pone `create_doctor_profile` (en las dos ramas: cuando reclama un borrador y cuando inserta de cero) en el momento exacto en que el doctor termina el registro y se le asigna `owner_user_id`. Hacía falta una columna nueva porque ni `created_date` (cuándo se creó el borrador, puede ser días antes de terminar) ni `updated_date` (se mueve solo con cada edición/autoguardado del panel) sirven para medir "cuánto lleva con su cuenta ya creada".
- **`specialist.profile_reminder_sent_at`**: columna nueva, idempotencia (mismo patrón que `recovery_email_N_sent_at`: se marca con un `UPDATE ... WHERE profile_reminder_sent_at IS NULL` antes de mandar el correo -- si esa actualización no "gana" la fila, no se manda dos veces).
- **`public.send_profile_completion_reminder_emails()`**: la corre `pg_cron` cada hora (job `send-profile-completion-reminder-emails` -- no hace falta más precisión para un disparador de "1 día"). Candidatos: `owner_user_id is not null`, `account_created_at` hace ≥1 día, `profile_reminder_sent_at is null`, **y `completeness_score < 90`** (si el doctor ya llenó casi todo su perfil solo, no tiene caso mandarle el recordatorio). Reusa `registration_recovery_email_html(...)` tal cual (misma plantilla que el §8g) con el botón apuntando a `/panel-medico` (no al registro -- este doctor ya tiene sesión) y la foto de la doctora morena (`hero-registro-recuperacion-doctor-2.jpg`, la misma del correo 2 de recuperación). Manda con el tipo nuevo `recordatorio_completar_perfil` (agregado a la lista blanca de `send_transactional_email`, respeta la preferencia de correo "estado" del doctor, igual que los demás correos de estado).
- **El pie de la plantilla compartida se volvió neutral** ("Recibiste este correo relacionado con tu cuenta en BuscoUnDoctor" en vez de "...porque comenzaste un registro") para que sirviera también para este correo sin tener que agregarle un parámetro nuevo a la función (evita el problema de firmas huérfanas ya documentado arriba).
...
- **Los botones solo se muestran si el proveedor ya está configurado en Supabase.** `RegistroMedico.jsx` le pregunta a Supabase al cargar la página (`GET /auth/v1/settings`, endpoint público, no necesita sesión) qué proveedores están prendidos (`external.google`, `external.azure`) y solo pinta el botón correspondiente si es `true`. Esto es a propósito: sin este checkeo, un doctor real habría podido caer en una pantalla de error en inglés de Supabase ("Unsupported provider") si le daba clic a un botón sin configurar -- con esto, el botón aparece solo, automáticamente, el día que se configure la llave, sin tener que tocar código ni volver a publicar el sitio.
- **`src/lib/supabaseClient.js`** ahora exporta `supabaseUrl` y `supabaseAnonKey` (antes solo el cliente ya armado) para poder llamar ese endpoint de settings desde `RegistroMedico.jsx`.
- **Flujo del lado de código (ya existía, solo se conectó):** al darle clic, se guarda todo lo que el wizard llevaba capturado (`data`) en `localStorage` (`buscoundoctor_pending_registro`) y se llama `base44.auth.loginWithProvider('google' | 'microsoft', returnUrl)`, que hace `supabase.auth.signInWithOAuth(...)` -- esto navega el navegador completo a Google/Microsoft y de regreso a `/registro-medico` ya autenticado. El `useEffect` de arriba de `RegistroMedico.jsx` (que ya existía) detecta la sesión, recoge lo guardado en `localStorage` y llama a `createDoctorProfile` solo, sin pedir nada de nuevo -- nunca hace falta el paso de código de verificación (Google/Microsoft ya confirmaron el correo). Se corrigió de paso un hueco: si esa creación fallara (ej. cédula duplicada), ahora se recupera lo que ya había escrito en vez de mandarlo a un formulario en blanco.
- **El correo de contacto (`specialist.email`) no depende de lo que regrese Google/Microsoft.** Ya se capturó en el paso 1 del wizard y se guardó en el borrador desde antes de tocar el botón -- `create_doctor_profile` nunca lo pisa. Por eso, en Microsoft, se activó a propósito el interruptor **"Allow users without an email"** en Supabase (algunas cuentas corporativas restringidas no mandan el correo): como de todos modos ya tenemos el correo del wizard, es más seguro dejar entrar a ese doctor que bloquearlo sin necesidad.
- **Las llaves (Client ID/Secret) viven en las cuentas personales de Google Cloud / Microsoft de quien las configuró** (hoy, Jorge) y se pegan directo en Supabase (Authentication → Providers → Google / Azure) -- nunca en el repo ni en el `.env`. Callback URL para ambos: `https://iiklgyzyvbrtrxjfucoc.supabase.co/auth/v1/callback`.
- **Azure (Microsoft) necesita, además del Client ID/Secret:** el tipo de cuenta del registro de la app en Microsoft Entra debe ser **"Cualquier inquilino de Entra ID + cuentas personales de Microsoft"** (si no, se quedaría fuera cualquier doctor con Outlook/Hotmail personal, o cualquiera con cuenta de una organización distinta a la de quien configuró la app) y en Supabase el campo **"Azure Tenant URL"** debe ser `https://login.microsoftonline.com/common` (no solo la palabra "common" -- pide una URL completa). **El secreto de Microsoft caduca** (a diferencia del de Google): hay que renovarlo en Entra ID → el registro de la app → Certificados y secretos antes de que venza, o el botón deja de funcionar.
- **Aviso conocido, no bloqueante:** Microsoft avisa que "apps nuevas de varios inquilinos sin publisher verificado" pueden tener restricciones con algunas cuentas corporativas muy estrictas (poco común entre doctores individuales, casi todos con Outlook/Hotmail personal). Verificar el publisher ante Microsoft es un trámite aparte -- pendiente a futuro solo si se vuelve un problema real.
...
- **Confirmaciones "¿seguro?" del admin dueños:** usa `useConfirmDialog()` (`src/hooks/useConfirmDialog.js`) + `<ConfirmDialog {...dialogProps} />` (`src/components/admin/ConfirmDialog.jsx`) — nunca `confirm()`/`window.confirm()` del navegador (se reemplazaron los 16 que quedaban, en 11 pantallas, el 2026-09-22). Patrón: `const { confirm, dialogProps } = useConfirmDialog();` arriba del componente, `const ok = await confirm({ title, description, confirmLabel }); if (!ok) return;` donde antes había un `confirm(...)`, y `<ConfirmDialog {...dialogProps} />` una vez en el JSX. `prompt()` (el único que queda, en `AdminDoctores.jsx` para el motivo opcional de la papelera) no se tocó — reemplazarlo necesitaría un modal con campo de texto, no solo confirmar/cancelar.
